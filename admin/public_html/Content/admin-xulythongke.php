<?php
    ...
    $result=$con->selectsql("donhang as dh,chitietdonhang as ct,sanpham as sp","tensanpham,sum(soluong) as tongsoluongban","where sp.tinhtrang=1 and (thoigiandat BETWEEN '$start' and '$end')
        ...
        if(isset($_POST["Category"])){
            if($_POST["Category"]==0){
                $result=$con->selectsql("donhang as dh,chitietdonhang as ct,sanpham as sp","thoigiandat,tensanpham,soluong","where sp.tinhtrang=1 and dh.madonhang=ct.madonhang and sp.masanpham=ct.masanpham");
                if($result->num_rows>0){
                    $i=1;
                    while ($row = $result->fetch_assoc())
                    {
                ...
                $result=$con->selectsql("donhang as dh,chitietdonhang as ct,sanpham as sp,chitietsanpham as ctsp,theloai as tl","thoigiandat,tensanpham,soluong","where sp.tinhtrang=1
